...
import pandas as pd
df=dict()
df['m49']= pd.read_csv('m49regin_country.tsv', sep='\t', encoding="utf8", keep_default_na=False, na_values=[''])           #, keep_default_na=False, na_values=['']
df['cldr']= pd.read_csv('Unicode_UN.tsv', sep='\t', encoding="utf8", keep_default_na=False, na_values=[''])  #list(df['cldr']['numeric'])
...
grouped_table.to_csv("_cf_m49_cldr_{}.tsv".format(label), sep='\t', encoding="utf8", index=False, na_rep='缺失值', columns=['Category','Number','Countries'])
...
df[label].sort_values(['region','countrycode2']).to_csv("_cf_m49_cldr_{}.tsv".format(label), sep='\t', encoding="utf8", index=False, na_rep='缺失值')
...
df_.to_csv("_cf_m49_cldr_{}.tsv".format(label), sep='\t', encoding="utf8", index=True, na_rep='缺失值')

list_col_zh=["數字代碼","三字母碼", "二字母碼", "M49國家英文名", "CLDR國家英文名", "M49國家分類", "CLDR國家分類", ]
df_.columns=list_col_zh
df_.to_csv("_cf_m49_cldr_{}_zh.tsv".format(label), sep='\t', encoding="utf8", index=False, na_rep='缺失值')


label='_region_categorization_diff'
list_col=["countrycode", "countrycode2", "countryname_left", "countryname_right", "region_left", "region_right", ]
df_=df[label].sort_values(['region_left','region_right','countrycode2'])
df_.to_csv("_cf_m49_cldr_{}.tsv".format(label), sep='\t', encoding="utf8", index=False, na_rep='缺失值')
...
result.to_csv("_cf_m49region_Unicode_UN.tsv", sep='\t', encoding="utf8", index=False)
